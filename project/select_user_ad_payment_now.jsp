...
<%
    ...
    String WhereDefault = "select a.id_ad, a.ad_title, a.ad_url, a.ad_image, a.ad_price, a.ad_location, a.ad_indate, datediff(a.ad_outdate, now()), p.payment_indate, p.payment_outdate, p.id_payment from ad as a inner join payment as p on a.id_ad=p.ad_id_ad where a.user_email='"+email+"' and p.payment_outdate is null and a.ad_checkdate is not null and date(a.ad_indate) < date_format(now(), '%Y-%m-%d') and date(a.ad_outdate) > date_format(now(), '%Y-%m-%d')";
    int count = 0;
